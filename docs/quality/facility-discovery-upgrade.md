...
- Diagnosis ranking accepts only finite numeric values in [0, 1] from the known analysis/session contract. Each collapsed disease row shows `Điểm gợi ý: N%`; the section explicitly identifies these as AI-suggested diseases for discussion with a doctor. A visible note distinguishes the score from disease probability, with additional details on request. Missing or invalid scores say `Chưa có điểm gợi ý`, never an invented zero. No new medical reasoning is generated in the frontend.
...
- Initial upgrade validation included the map, feedback-review and symptom-analysis UI suites: 45 browser tests passed, including 14 cases in the upgrade suite. All 52 unit tests, repository-wide lint and the production build passed.
- Percentage-display follow-up: 42 tests passed across the map and upgrade suites, including visible scores before opening disease details at 320/390/1440 px, the explanation toggle and absent/invalid scores without fabricated percentages. Sidebar accessibility checks, repository-wide lint and the production build passed. Scores use the existing backend value; neither the algorithm nor stored medical information is changed.
