...
- Production sends same-origin `/api/*` requests through the Vercel rewrite generated by `vercel.ts` from `BACKEND_API_ORIGIN`. If that environment variable is not configured, the frontend still builds but `/api/*` is not proxied to a backend.
