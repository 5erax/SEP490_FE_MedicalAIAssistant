...
async function openOtpModalAndFillCode(page, digits = "123456") {
  await page.getByRole("tab", { name: "Bảo mật" }).click();
  await page.getByRole("button", { name: "Gửi mã xác thực qua email" }).click();
  const dialog = page.getByRole("dialog", { name: "Xác thực qua email" });
...
test("security tab: confirming requires all 6 digits before closing the modal", async ({ page }) => {
  await openPatientProfile(page);
  ...
  const dialog = await openOtpModalAndFillCode(page, "123");
  await dialog.getByRole("button", { name: "Xác nhận" }).click();

  await expect(dialog.getByText("Vui lòng nhập đủ 6 số.", { exact: true })).toBeVisible();
  await expect(dialog.locator(".otp-box")).toHaveCount(6);
});

test("security tab: confirming the OTP closes the modal and the page shows only the new-password fields", async ({ page }) => {
  await openPatientProfile(page);
  await page.route("**/api/authentication/forgot-password", (route) => route.fulfill({
    contentType: "application/json",
    body: JSON.stringify({ success: true, data: null }),
  ...
  const dialog = await openOtpModalAndFillCode(page);
  await dialog.getByRole("button", { name: "Xác nhận" }).click();
...
test("security tab: submitting the reduced form with the confirmed OTP changes the password", async ({ page }) => {
...
test("security tab: a wrong/expired OTP reopens the modal with the error there, keeping the typed password", async ({ page }) => {
  await openPatientProfile(page);
  await page.route("**/api/authentication/forgot-password", (route) => route.fulfill({
    contentType: "application/json",
    body: JSON.stringify({ success: true, data: null }),
  ...
  const dialog = await openOtpModalAndFillCode(page, "000000");
  await dialog.getByRole("button", { name: "Xác nhận" }).click();
  await page.getByLabel(/^Mật khẩu mới/).fill("NewPass456!");
  await page.getByLabel("Nhập lại mật khẩu mới").fill("NewPass456!");
  await page.getByRole("button", { name: "Lưu mật khẩu mới" }).click();

  // The modal reopens with the error right below the OTP boxes, never on
  // the page where the password fields live.
  await expect(dialog).toBeVisible();
  await expect(dialog.locator(".otp-box")).toHaveCount(6);
  await expect(dialog.getByText("Mã xác thực không hợp lệ hoặc đã hết hạn.", { exact: true })).toBeVisible();
  await expect(page.locator(".profile-card .security-message-error")).toHaveCount(0);

  // Fixing the code and confirming again should not require retyping the
  // password.
  const boxes = dialog.locator(".otp-box");
...
test("security tab: a password-policy error stays on the page and does not reopen the OTP modal", async ({ page }) => {
  await openPatientProfile(page);
  await page.route("**/api/authentication/forgot-password", (route) => route.fulfill({
    contentType: "application/json",
    body: JSON.stringify({ success: true, data: null }),
  ...
  await page.route("**/api/authentication/change-password", (route) => route.fulfill({
    status: 400,
    contentType: "application/json",
    body: JSON.stringify({ success: false, message: "Passwords must be at least 8 characters.", data: null }),
  }));

  const dialog = await openOtpModalAndFillCode(page);
  await dialog.getByRole("button", { name: "Xác nhận" }).click();
  await page.getByLabel(/^Mật khẩu mới/).fill("short1!");
  await page.getByLabel("Nhập lại mật khẩu mới").fill("short1!");
  await page.getByRole("button", { name: "Lưu mật khẩu mới" }).click();

  await expect(dialog).toBeHidden();
  await expect(page.getByRole("dialog", { name: "Xác thực qua email" })).toHaveCount(0);
  await expect(page.getByLabel(/^Mật khẩu mới/)).toBeVisible();
  await expect(page.getByText("Mật khẩu phải có ít nhất 8 ký tự.", { exact: true })).toBeVisible();
});
