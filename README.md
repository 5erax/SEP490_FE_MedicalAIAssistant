...
Màn hình thuốc cho phép người dùng quản lý thông tin thuốc tự khai báo và lịch nhắc qua
`/api/user-medications/*`. Phân tích xét nghiệm sử dụng các API `/api/lab-tests/*` và dữ
liệu hồ sơ của tài khoản đang đăng nhập.
...
VITE_REALTIME_BASE_URL=https://your-api-domain.example.com
...
| `VITE_REALTIME_BASE_URL` | Khi dùng cập nhật kế hoạch phục hồi theo thời gian thực | HTTPS origin công khai của backend chứa hub `/hubs/recovery-plans`. Local development tự dùng `VITE_API_BASE_URL` nếu để trống; production cần HTTPS để tránh mixed content. |
